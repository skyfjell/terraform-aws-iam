...
| <a name="input_role_config"></a> [role\_config](#input\_role\_config) | Builtin role configurations exposed. `admin` and `read-only` share same type as `global` but will override `global`. | <pre>object({<br>    global = optional(object({<br>      max_session_duration = optional(number, 1)<br>    }))<br>    admin     = optional(map(any))<br>    read_only = optional(map(any))<br>  })</pre> | `{}` | no |
